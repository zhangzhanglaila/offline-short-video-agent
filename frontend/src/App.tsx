...
function App() {
  ...
  // Schedule auto-save on track changes (debounced 1.5s)
  useEffect(() => {
  ...
  // Load saved session with undo/redo stacks
  const loadSavedSession = useCallback(async (sessionId: string) => {
    setLoading(true);
    setError(null);
    try {
      // Try loading saved state first (includes undo/redo)
      ...
      // Fall back to live timeline from thinking session
  ...
  // Fall back to demo data if no sessions
  useEffect(() => {
